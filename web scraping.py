...
DOWNLOAD_TREATIES = False
DOWNLOAD_TREATY_DECISIONS = False

PAGE_START_TREATY_DECISIONS = 20
PAGE_STOP_TREATY_DECISIONS = 20 #out of 775

PAGE_START_TREATIES = 1
...
def download_pdfs(liste):
    ...
    for info in pdf_inf:
        ...
        add, title_of_treaty = download_layer_two(web_site, download_folder) #download and return if one pdf was downloaded (1) or none (0) 
        doc_counter += add
        
        #write meta data
        new_meta_data["Title"] = title_of_treaty #add title
        ...
        if 'Abstract' in new_meta_data:
            #strip html tags
            clean = re.compile('<.*?>')
            text = re.sub(clean, '', str(new_meta_data['Abstract']))
            
            with open(f"Downloads/{download_folder}/{title_of_treaty}.txt", "w", encoding='utf-8') as f:
                f.write(text)
...
def download_layer_two(website, download_folder):
    """
    ...
    print(title)
    title = re.sub('[^0-9,^A-Z,^a-z, ]','',title)
    if len(title) > 200: title = title[0:200] #due to apparent maximum of name length a pdf can have
...
def download_treaties(html_text):
    ...
    for info in html_text:
        ...
        #actually download pdf from the website
        web_site = request.urlopen(link).read()
        add, title_of_treaty = download_treaty(web_site) #download and return if one pdf was downloaded (1) or none (0) 
        doc_counter += add
...
if __name__ == "__main__":
    ...
    if DOWNLOAD_TREATY_DECISIONS:
        for page_num in range(PAGE_START_TREATY_DECISIONS, PAGE_STOP_TREATY_DECISIONS + 1):
            res = open_website(f"https://www.ecolex.org/result/?type=treaty&type=decision&page={page_num}")
